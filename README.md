...
访问 [alphapai-web.rabyte.cn](https://alphapai-web.rabyte.cn/reading/paiwork) 注册账号。首次使用时需要在浏览器中手动登录。
...
> 浏览器窗口打开后，**手动完成登录**。登录后告诉 AI "已登录"即可继续。

### 第二步：确认登录状态

```bash
browser-use --session paipai eval "document.querySelector('textarea') ? 'OK_LOGGED_IN' : 'NEED_LOGIN'"
```
...
**Q: 登录态会丢失吗？**
A: 使用 `--session paipai` 参数，同一 session 的登录态会持久化，重启浏览器不丢失。
