...
#### 方法0（⭐ 最佳方案）：直接输入 `/技能名` 唤醒

> **实战验证**：7/7 测试成功！最简洁可靠的方案。
> 直接在 textarea 输入 `/观点challenge` 即可自动唤醒技能，无需任何 click 操作。
> 适用于**所有** PaiWork Skills 面板中列出的技能（`/技能名` 格式）。
...
# Step 2: 直接输入 /技能名 + 参数，一次性完成唤醒+传参
# 例如：/观点challenge 对今日原油市场提出观点并实施挑战
# JS setter 输入完整指令（type 命令不稳定，用 setter 最可靠）
cat > /tmp/paipai_skill_wake.js << 'JSEOF'
var t = document.querySelector('textarea');
var setter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value').set;
setter.call(t, '/观点challenge 你的挑战/分析内容...');
t.dispatchEvent(new Event('input', { bubbles: true }));
t.dispatchEvent(new Event('change', { bubbles: true }));
t.focus();
'done: ' + t.value;
JSEOF
browser-use --session paipai eval "$(cat /tmp/paipai_skill_wake.js)"
sleep 1
...
> **✅ 验证结果（7/7）**：技能成功激活，PaiWork 走 fact-debate 辩论框架，
> 生成多空对照式分析（185秒），与普通对话模式（60-120秒）明显不同。
>
> **通用性**：将 `观点challenge` 替换为任意技能名即可激活对应技能，如：
> - `/浏览器代理` → 浏览器代理技能
> - `/有道云笔记同步` → 有道云笔记同步技能
> - 技能名需与 Skills 面板中显示的名称一致（不区分大小写）
...
> **⚠️ 强制规则：信息时效性检查（实战教训）**
>
> 搜索获得的每一条信息**必须标注发布日期**，并判断它属于以下哪个时间窗口：
>
> | 标签 | 定义 | 处理方式 |
> |------|------|---------|
> | 🟢 **实时** | 当天/本周发布 | 可直接引用 |
> | 🟡 **近期** | 1个月内 | 可引用但需注明"截至X月X日" |
> | 🟠 **滞后** | 1-3个月 | **必须标注日期**，说明可能已过时 |
> | 🔴 **过时** | 3个月以上 | **禁止作为当前判断依据**，仅可做历史对比 |
>
> **特别警惕「结构性断点」**——如果分析期间发生了重大事件（战争、制裁、政策突变、 pandemic 等），
> 则该事件**之前**的所有数据自动降级为 🔴，只能作为"战前/事件前基线"使用，**不能代表当前格局**。
>
> **实战教训**：在分析亚洲成品油时，引用了3月5日的裂解价差数据（$48/桶）来质疑 PaiPai 的"柴油裂突破150美元"，
> 但完全忽略了2月28日美伊战争导致的霍尔木兹海峡封锁——战后成品油价格已暴涨20-31%，
> 3月数据本身就是受战争影响的早期快照，不能反映6月和平协议后的最新格局。
> 被用户指出"3/5日数据为战前数据，没有考虑到战争对亚洲市场成品油的影响"。
>
> **操作规范**：
> 1. WebSearch 每条结果在笔记中记录 `[来源 | 日期 | 时效标签]`
> 2. WebFetch 获取全文时，**优先提取文章发布日期**（通常在标题下方或文末）
> 3. 引用数据时必须带上日期：`"据 [机构] [日期] 数据/报告"`
> 4. 如果搜索结果多数为旧数据，**主动追加搜索** `[关键词] [当前月份] latest 2026`
> 5. 分析结论前自查：**「我引用的最新数据是哪天的？期间有没有发生改变格局的大事件？」**

**深度提取**——对关键搜索结果用 WebFetch 获取全文：
- 机构研报全文（中金、华泰等公众号文章）
- 权威数据源（IEA/EIA 月报、OPEC 月报等）
- 实时数据页面（Trading Economics、金投网等）
- **WebFetch prompt 中必须包含**：`"提取文章发布日期和所有关键数据的统计截止日期"`
...
| **⚠️ 时效性盲点** | **自己引用的数据是否也是过时的？** 是否存在结构性断点（战争/制裁/政策突变）导致旧数据失效？自己引用的"最新数据"日期距离今天多久？期间是否发生了改变格局的大事件？（详见 Phase 1 时效性检查规则）|

> **输出**：生成结构化的差异点列表，每条标注 `[来源 | 日期 | 🟢🟡🟠🔴时效标签]`。
